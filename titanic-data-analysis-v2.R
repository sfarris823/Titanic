...
########################################################################
m1.combined <- rbind(  
            combined[combined$Pclass==1 & (combined$SimpleTitle=="Master" | combined$SimpleTitle=="Mrs" | combined$SimpleTitle=="Miss"),], 
            combined[combined$Pclass==2,]
           )
m1.train <- m1.combined[m1.combined$Survived!="Unk",]
m1.test  <- m1.combined[m1.combined$Survived=="Unk",]
...
# View the results of the random forest, OOB estimate of error rate is 6.76%.  93.24% accuracy.
m1.rf
...
m2.combined <- rbind(  
            combined[combined$Pclass==1 & !(combined$SimpleTitle=="Master" | combined$SimpleTitle=="Mrs" | combined$SimpleTitle=="Miss"),], 
            combined[combined$Pclass==3,]
          )
m2.train <- m2.combined[m2.combined$Survived!="Unk",]
m2.test  <- m2.combined[m2.combined$Survived=="Unk",]
...
# View the results of the random forest, OOB estimate of error rate is 19.34%.  80.66% accuracy.
m2.rf
# Draw a Variable Importance Plot.
varImpPlot(m2.rf)
